...
cascade, and SDK method selection.
"""
...
def _load_script(name: str):
    ...
    fake_exa.Exa = MagicMock()
    sys.modules[name] = module
    with patch.dict(sys.modules, {"exa_py": fake_exa}):
...
class ExaFlowTests(unittest.TestCase):
    """End-to-end: run() creates the client and calls the right SDK method."""
    ...
    def test_no_integration_header_is_set(self):
        _, client, _ = self._run_with_mock(["what is attention", "--highlights"])
        self.assertNotIn("x-exa-integration", client.headers)
